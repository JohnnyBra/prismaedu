...
const AuthView: React.FC = () => {
  ...
  const renderModeSelect = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300 relative w-full max-w-md">
      <div className="text-center mb-8 flex flex-col items-center">
        ...
        <h1 className="text-4xl font-extrabold text-brand-900 drop-shadow-sm mb-2 tracking-tight">Prisma</h1>
        <p className="text-brand-600 text-lg font-medium">Cooperativa de Enseñanza La Hispanidad</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <button 
          ...
          className="p-8 glass-panel rounded-2xl shadow-glass hover:shadow-glass-hover hover:scale-105 transition-all flex flex-col items-center gap-4 group"
        >
          <div className="w-20 h-20 bg-brand-50 rounded-full flex items-center justify-center text-brand-600 group-hover:bg-brand-600 group-hover:text-white transition-colors">
            <School size={40} />
          </div>
        ...
        <button 
          ...
          className="p-8 glass-panel rounded-2xl shadow-glass hover:shadow-glass-hover hover:scale-105 transition-all flex flex-col items-center gap-4 group"
        >
          <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center text-orange-600 group-hover:bg-orange-600 group-hover:text-white transition-colors">
            <Home size={40} />
          </div>
      ...
      <div className="pt-8 text-center">
        <button 
          ...
          className="text-xs text-brand-500 hover:text-brand-700 font-bold flex items-center justify-center gap-1 mx-auto transition-colors"
        >
  ...
  const renderClassSelect = () => {
     return (
        <div className="w-full max-w-2xl animate-in fade-in slide-in-from-right-8 duration-300">
           <h2 className="text-3xl font-bold text-center text-brand-900 drop-shadow-sm mb-8 mt-12">Elige tu Clase</h2>
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {classes.map(cls => (
                 <button
                    ...
                    className="glass-panel p-6 rounded-xl shadow-glass hover:shadow-glass-hover hover:scale-105 transition-all border border-white/20 text-center"
                 >
                   <div className="w-16 h-16 mx-auto bg-brand-50 rounded-full flex items-center justify-center text-brand-600 mb-3">
                      <School size={28} />
                   </div>
  ...
  const renderGroupSelect = () => {
    ...
    return (
      <div className="w-full max-w-2xl animate-in fade-in slide-in-from-right-8 duration-300">
         <h2 className="text-3xl font-bold text-center text-brand-900 drop-shadow-sm mb-8 mt-12">Elige tu Familia</h2>
         {families.length === 0 ? (
            <div className="glass-panel p-8 rounded-xl text-center shadow-glass">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {families.map(fam => (
                   <button
                      ...
                      className="glass-panel p-6 rounded-xl shadow-glass hover:shadow-glass-hover hover:scale-105 transition-all border border-white/20 text-center"
                   >
                     <div className="w-16 h-16 mx-auto bg-orange-50 rounded-full flex items-center justify-center text-orange-600 mb-3">
                        <Home size={28} />
                     </div>
  ...
  const renderUserSelect = () => {
    ...
    return (
      <div className="w-full max-w-4xl animate-in fade-in slide-in-from-right-8 duration-300">
        <h2 className="text-3xl font-bold text-center text-brand-900 drop-shadow-sm mb-8 mt-12">¿Quién eres?</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {filteredUsers.map(user => (
            <button
              ...
              className="glass-panel p-4 rounded-xl shadow-glass hover:shadow-glass-hover hover:-translate-y-2 transition-all flex flex-col items-center border border-white/20"
            >
              <div className="mb-3">
                 {user.role === Role.TUTOR || user.role === Role.PARENT || user.role === Role.ADMIN ? (
                    <div className={`w-20 h-20 rounded-full flex items-center justify-center ${user.role === Role.ADMIN ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-500'}`}>
  ...
  const renderPinEntry = () => (
    <div className="glass-panel rounded-2xl shadow-glass p-8 max-w-sm w-full animate-in zoom-in duration-300">
        <div className="text-center mb-6">
          <div className="mx-auto w-20 h-20 mb-4">
             {selectedUser?.role === Role.STUDENT ? (
                <Avatar config={selectedUser.avatarConfig} size={80} />
             ) : (
                <div className={`w-full h-full rounded-full flex items-center justify-center ${selectedUser?.role === Role.ADMIN ? 'bg-gray-800 text-white' : 'bg-brand-100 text-brand-600'}`}>
        ...
        <div className="flex justify-center gap-4 mb-8">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className={`w-12 h-14 rounded-lg border-2 flex items-center justify-center text-2xl font-bold transition-all ${pin[i] ? 'border-brand-500 bg-brand-50 text-brand-600' : 'border-gray-200 bg-white/50'}`}>
              {pin[i] ? '•' : ''}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
            <button
              ...
              className="h-14 rounded-xl bg-white/50 hover:bg-white/80 active:bg-white font-bold text-xl text-gray-700 transition-colors border border-white/20"
            >
              {num}
            </button>
          ))}
          ...
          <button
            ...
            className="h-14 rounded-xl bg-red-50 hover:bg-red-100 active:bg-red-200 flex items-center justify-center text-red-500 transition-colors border border-white/20"
          >
            <Delete size={24} />
          </button>
        </div>
        ...
        <button
          ...
          className="w-full py-4 rounded-xl bg-brand-600 hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-lg flex items-center justify-center gap-2 shadow-lg transition-all"
        >
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden">
       {/* Background is handled by .bg-mesh in index.html, so we are transparent here */}

       {step !== 'MODE_SELECT' && (
         <button 
           onClick={goBack}
           className="absolute top-6 left-6 text-brand-700 hover:bg-white/20 flex items-center gap-2 font-bold bg-white/10 px-4 py-2 rounded-full transition-all backdrop-blur-md z-50 border border-brand-200"
         >
       ...
       {/* Footer / Credits */}
       <footer className="mt-8 text-center text-brand-900/40 text-xs py-4 z-10">
