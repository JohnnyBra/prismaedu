...
import { GraduationCap, Home, User as UserIcon, Delete, ArrowRight, ArrowLeft, School, Shield, KeyRound } from 'lucide-react';
...
const AuthView: React.FC = () => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <button 
          onClick={() => {
            ...
            setStep('TEACHER_METHOD_SELECT'); // Direct to Method Select for teachers
          }}
  ...
  // STEP 1.1: Teacher Method Select
  const renderTeacherMethodSelect = () => (
    <div className="w-full max-w-2xl animate-in fade-in slide-in-from-right-8 duration-300">
        <h2 className="text-3xl font-bold text-center text-white drop-shadow-md mb-8 mt-12">Elige Método de Acceso</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-lg mx-auto">
            {/* Manual Login */}
            <button
               onClick={() => setStep('USER_SELECT')}
               className="bg-white/95 backdrop-blur-sm p-8 rounded-2xl shadow-xl hover:shadow-2xl hover:scale-105 transition-all border border-white/20 text-center flex flex-col items-center gap-4 group"
            >
               <div className="w-20 h-20 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                  <KeyRound size={40} />
               </div>
               <div>
                 <span className="block text-xl font-bold text-gray-800">Usuario y Contraseña</span>
                 <span className="text-sm text-gray-500">Acceso Manual</span>
               </div>
            </button>
            ...
            >
               ...
               <div>
                 <span className="block text-xl font-bold text-gray-800">Google</span>
                 <span className="text-sm text-gray-500">Cuenta Colegio</span>
               </div>
            </a>
        </div>
    </div>
